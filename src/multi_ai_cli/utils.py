...
Includes path security, editor integration, console safety, string
helpers, etc.
"""
...
def secure_resolve_path(
    ...
    Args:
        ...
        category (str): 'data' or 'efficient' to select the correct base
            path.
        config (configparser.ConfigParser | None): Configuration object
            containing paths.
...
def open_editor_for_prompt(
    ...
    """
    Opens the user's preferred editor with a temporary file.

    Uses the ``$EDITOR`` environment variable or falls back to ``vi``.
...
def print_welcome_banner(
    ...
    print("[*] GitHub:   @github.repo --repo owner/name")
    print("[*]           @github.tree --repo owner/name --path src/")
...
def _get_cfg_int(
    ...
    """
    Safely reads an integer from config with fallback on error.

    Args:
        config (configparser.ConfigParser): The configuration object to
            read from.
...
def _tail_of(text: str, n: int) -> str:
    """
    Returns the last ``n`` characters of text (or the whole text if shorter).
...
def extract_code_block(text: str) -> str:
    ...
    Used for ``-w:code`` mode to save only code, stripping explanatory
    text. Falls back to full text if no code blocks are found.
